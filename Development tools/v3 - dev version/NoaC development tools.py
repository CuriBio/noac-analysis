...
Intended as a development platform for new features this edition of the NoaC analysis tool has been broken into cells to allow easier segmented analysis and debugging.
Additionally this version will accept only a single file vs a multifile directory
NB: Ensure the correct utils version has been loaded before beginning 
"""
...
# %%check utils version and reset defaults
# check utils version
utils.print_version()
...
for key in filtered_channels:
    ...
    PeakletFinder = utils.Peaklets(traversal_order=filtered_channels[key], 
                                   peaks=channel_peaks, 
                                   max_sample_interval=1000,
                                   peaklet_irregularity=PEAKLET_IRREGULARITY)
