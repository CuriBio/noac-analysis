...
def print_version():
    print('Utils v2 - localised peak searching implemented')
...
class Peaklets(object):
    ...
    def __init__(self, traversal_order: list, peaks: dict, max_sample_interval: int, peaklet_irregularity: int):
        self.electrode_order = traversal_order
        self.peaks = peaks
        self.max_sample_interval = max_sample_interval
        self.peaklet_irregularity = peaklet_irregularity
    ...
    def fit(self, peaks: dict, recordings: pd.DataFrame):
        ...
        for i, electrode in enumerate(self.electrode_order):
            ...
            elif peaklets.shape[0] != 0:
                peaklets = self.__find_peaklets([peaklets, se_peaks])

            peaklets = self.__filter_peaklets(peaklets, np.asarray(time))
            
        if peaklets.shape[0] != 0:
    ...
    def __filter_peaklets(self, peaklets: NDArray, time: NDArray):
        ...
        """
        first_electrode_peaks = self.peaks[self.electrode_order[0]]
        
        # temporal filtering
        # empty filtered peaklet list
        fp = []
        
        # filter by first peak of peaklet to allow peak by peak filtering        
        for i,peak in enumerate(first_electrode_peaks):
            
            # match first index in the array to peak index to produce subset of peaklets relating to only one peak
            p = peaklets[np.in1d(peaklets[:,0], peak), :]
                
            # if the last peaklet in the list is being examined use index of end of recording not index to next peak as interval
            if i==len(first_electrode_peaks)-1:
                next_peak = time.shape[0]
            else:
                next_peak = first_electrode_peaks[i+1]
             
            # with very long delays random peaks can be linked as peaklets, this allows the user to add an upper limit to the 
            # peaklet span to reduce spurious peaklets
            max_inter_peak_interval = next_peak-peak if next_peak-peak < self.max_sample_interval else self.max_sample_interval
                        
            # filter peaklets according to max sample interval identified above
            valid_peaklets_temporal = (p.max(1) - p.min(1)) <= (max_inter_peak_interval)
            p = p[valid_peaklets_temporal]

            # if more than 1 peaklet passes temporal filtering return only the fastest peaklet
            if p.shape[0] > 1:
                valid_peaklets_temporal = np.argmin((p.max(1) - p.min(1)))
                p = p[valid_peaklets_temporal]
            
            # if sample interval between peaks in peaklets set is highly irregular this is likely to be a false peaklet.
            # filter to remove peaklets with irregular intervals, only allow peaklets where interpeak intervals vary by less than the number of samples given by peaklet_irregularity
            if p.shape[0] != 0 and np.diff(p).size != 0:
                valid_peaklets_temporal = np.diff(p).max()-np.diff(p).min() <= self.peaklet_irregularity
                p=p.squeeze()[valid_peaklets_temporal]
...
class Peaks(object):
    ...
    def __find_peaks(self, 
        ...
        # remove all signal 100 samples either side of identified peaks
        p_window = np.array([])
        
        for peak in peak_indices:    
            p=np.array([peak+i for i in range(-100,101) if peak+i<len(Y) and peak+i>0])
            p_window=np.concatenate([p_window,p], axis=None)
            
        p_window=p_window.astype(int)
        
        # remove any values from voltage array which are within a peak window
        Y_no_peaks=np.delete(np.asarray(Y), list(p_window))
        
        # extend the voltage array to match original size (duplicates data) to ensure std is no effected by large changes in size
        Y_no_peaks=np.resize(Y_no_peaks, len(Y))
                
        # get noise std
        Y_no_peaks_std=Y_no_peaks.std()
        
        # refind peaks with noise std not total std
        peak_indices, _ = signal.find_peaks(Y,
                                            height=Y.mean() + height_factor*Y_no_peaks_std,
                                            width=min_samples_between_twitches/width_factor,
                                            distance=min_samples_between_twitches,
                                            prominence=Y_no_peaks_std*prominence_factor)
