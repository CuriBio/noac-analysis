...
v3 - Peak finding in associated utils uses two stage std() based approach and peaklet search depends upon peak by peak localised searching
"""
...
#%% TODO move this to utils and tidy up as appropriate
def single_file_complete_analysis(max_twitch_frequency : int, 
                                  prominence_factor : int, 
                                  width_factor : int, 
                                  height_factor : int, 
                                  direction : str, 
                                  peaklet_irregularity : int,
                                  file_location : str):
    
    recording_file = file_location
    
    directory_name = recording_file.split('\\')[-1].split('.')[0]
    
    # Create output directories
    # empty output directories
    dir_list = [f'./data/results - {directory_name}/',
                f'./data/results - {directory_name}/plots/',
                f'./data/results - {directory_name}/traces/',
                f'./data/results - {directory_name}/xlsx/']
    ...
    PeakFinder = utils.Peaks(max_twitch_frequency=max_twitch_frequency, 
                             prominence_factor=prominence_factor, 
                             width_factor=prominence_factor, 
                             height_factor=height_factor)
    ...
    for key in filtered_channels:
        ...
        PeakletFinder = utils.Peaklets(traversal_order=filtered_channels[key], peaks=channel_peaks, max_sample_interval=1000, peaklet_irregularity=peaklet_irregularity)
    ...
    with pd.ExcelWriter(f'./data/results - {directory_name}/xlsx/Conduction_Velocity_Summary.xlsx') as writer:
            for key, value in filtered_summary.items():
                value.to_excel(writer, key)
    
    # save all metrics value dataframe to xlsx
    with pd.ExcelWriter(f'./data/results - {directory_name}/xlsx/Conduction_Velocities.xlsx') as writer:
    ...
    for key in filtered_channels:
        ...
        plt.savefig(f'./data/results - {directory_name}/traces/Peak Traces for {key}.png', bbox_inches='tight')
    ...
    with pd.ExcelWriter(f'./data/results - {directory_name}/xlsx/Burst_Metrics.xlsx') as writer:
    ...
    for key in filtered_channels:
        ...
        for i, electrode in enumerate(filtered_channels[key]):
            ...
            # save figure as .png
            plt.savefig(f'./data/results - {directory_name}/traces/Bursts and peaks for {key}.png',
                        bbox_inches='tight')
    ...
    for key in max_peak_index:
        ...
        F.savefig(f'./data/results - {directory_name}/traces/Peaklet from {key} - Peaklet no. {max_peak_index[key]}.png', bbox_inches='tight')
        plt.show()
    ...
    for key in sequential_pairs_dict:
        ...
        colmap = np.where(velocity_plot['Delay']>velocity_plot['Delay'].mean()+4*velocity_plot['Delay'].std(), 'r', 'k')
...
for i,f in enumerate(file_list):
    print(f'Analysis of file {i+1} of {len(file_list)} begining....')
    
    single_file_complete_analysis(MAX_TWITCH_FREQUENCY, 
                                  PROMINENCE_FACTOR,
                                  WIDTH_FACTOR, 
                                  HEIGHT_FACTOR, 
                                  DIRECTION, 
                                  PEAKLET_IRREGULARITY,
                                  f)
    plt.close('all')
    
    print(f'Analysis of file {i+1} of {len(file_list)} completed')